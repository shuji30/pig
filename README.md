...
| **家具でできること** | 家具をクリックすると、その家具でできることが帯に並びます。**ベッドで ねる**（横になります）／**テレビを みる**／**かがみに うつる**／**本を よむ**（本棚・キュリオ）／**ちくおんきを かける**（踊りだします）／**はちうえに みずやり**。もう一度おすか、歩き出すとやめます |
...
| **スタンプ** | 「きもち」の下段から10種。**吹き出しに絵柄が1枚**出ます（絵文字ではなく描いているので端末で見た目が変わりません）。人の部屋でも出せます |
...
`*.test.ts` は同じ階層に置いてあります（`npm test` で 266 件）。
