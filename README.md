...
- 部屋サイズの可変化と `SaveData` の複数部屋化を**最初に**やる（あとから分割すると全機能に波及する）
- 壁面レイヤーとリカラー。同じ形から色違いが作れるので**供給効率が最も高い**
