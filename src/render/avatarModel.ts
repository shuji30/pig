 ...
 *
 * たて: 足もとの行（`ROOM_GROUND`）より上が背の高さ 55.4px × 1.6 = 89px。
 * 腕を上げるモーションと髪のぶんに余りを見て 110。
 *
 * **足もとの行より下も要る**。すわると腰が座面（＝足もとの行）まで
 * 下がるので、すねと足はその下へ出る。下ぶんが足りないと絵から
 * はみ出して**足が消える**（実際に踏んだ）。腰の高さ 28px × 1.6 = 45px に
 * 余りを見て 56。
 */
export const ROOM_W = 88;
export const ROOM_GROUND = 110;
export const ROOM_H = ROOM_GROUND + 56;
