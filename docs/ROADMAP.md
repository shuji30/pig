...
| v0.4 | そだてる | 🔶 4/6 | ✅ 部屋の拡張（12→14→16→20）、✅ `SaveData` の複数部屋化、✅ 壁面レイヤー（10種）、✅ 家具のリカラー／未：床の部分張り替え、部屋テーマ |
...
- ✅ ユニットテスト 105 件（`core/wall`、壁の重なり判定、版の移行、リカラーの検証を追加）
...
1. 家具どうしの重なり順をトポロジカルソートへ（**大型家具を増やす前に**）
2. 床の部分張り替えと、部屋テーマのプリセット（ロココ / 和 / カフェ / パステル）
3. そのあと **v0.4.5 とびだす**（ロケット／月コロニー）。土台の複数部屋化は済んでいる
...
| 家具が増えて描画順が崩れる | 中 | 大型家具を増やす前にトポロジカルソート化（**まだ残っている**） |
