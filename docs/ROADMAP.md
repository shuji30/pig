...
**v0.4 は完了しました。次は v0.4.5「とびだす」（ロケット → 月コロニー）です。**
前提はすべて片づいています：

- `SaveData` の複数部屋化（`rooms` に1件足すだけ）
- 部屋ごとの広さ（月は地上と違う広さにできる）
- 重なり順のトポロジカルソート（大型家具のロケットを入れられる）
- 共有 URL（月の部屋もそのまま共有できる）

S 段（ロケット家具・発進演出・月コロニー・月家具6〜8種・帰る導線）から入ります。
