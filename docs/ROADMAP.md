...
## 付録：3Dモデル → 等角スプライト化（床の家具は完了）

「Unity に移してリアルにできるか」への答えとして、**Unity に移らずに絵の質を上げる道**を採った。
床に置く家具 66種をすべて 3Dモデルから焼いた絵に置き換えた（壁に掛けるものは未了）。
...
| 容量 | 212枚 / **1.37MB**（形の共有と、色を焼かないぶんで試作時の見積 2.2MB より小さい） |
...
- 壁に掛けるもの19種（`wallShape` 10種）。壁の平面 `(u,h)` は床と射影が別なので、別の焼き方が要る
