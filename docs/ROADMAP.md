...
| **v0.4.5** | **とびだす** | ✅ **S段 完了** | ロケット家具 → 月コロニー（14×14 の2つめの部屋）、低重力歩行、月家具10種／未：月面限定ミッション、共有月面広場（L段） |
...
1. **月面限定ミッション**（M段）。「行き先」がコンテンツの穴埋めにならないよう、
   月にも毎日の目的を置く
2. 判断ゲートの計測：**月の部屋の設置家具数の中位値が1週間で8個に届くか**。
   届かないなら行き先を増やすのは止めて、1部屋の密度に投資を戻す
3. 届いたら v0.5「いきている」の残り（家具インタラクション、時間帯、ペット、スタンプ）へ
...
| 🔶 **M 本命** | ＋1週 | ✅ 低重力歩行、✅ 地球が見える窓、✅ 常時星空（星空の壁）／未：月面限定ミッション、月面限定家具の入手経路 |
