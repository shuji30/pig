...
| v0.4 | そだてる | 🔶 2/6 | ✅ 部屋の拡張（12→14→16→20、コインで解放）、✅ `SaveData` の複数部屋化／未：壁面レイヤー、リカラー、床の部分張り替え、部屋テーマ |
...
- ✅ ユニットテスト 74 件（`state/save` の版の移行と `state/economy` の拡張を追加）
...
1. **壁面レイヤー**（窓・絵画・壁時計・壁掛け照明）。等角の箱庭で「部屋らしさ」がいちばん伸びる
2. **家具のリカラー**。1種から5〜6色が作れるので供給効率が最も高い
3. 家具どうしの重なり順をトポロジカルソートへ（**大型家具・壁面家具を増やす前に**）
4. そのあと **v0.4.5 とびだす**（ロケット／月コロニー）。土台の複数部屋化は済んでいる
...
- ✅ **`SaveData` の複数部屋化は v0.4 で済んでいる**（version 4、`rooms` ＋ `currentRoom`）。
  月コロニーは `rooms` に1件足し、`RoomData.size` に月の広さを入れるだけで置ける
- 部屋の広さも部屋ごとに持っているので、月を 14×14 など地上と違う形にできる
