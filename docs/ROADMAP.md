...
## 付録：3Dモデル → 等角スプライト化（案Aの試作）

「Unity に移してリアルにできるか」への答えとして、**Unity に移らずに絵の質を上げる道**を
1種だけ実装して測った。結論：**やる価値はあるが、全部まとめてやるしかない。**

### やったこと

- `tools/sprite-render/` に書き出しの道具を作った。ヘッドレス Chromium の中で Three.js を動かし、
  ロココいすを3Dモデル（猫脚は本物の回転体の曲線、角丸の座面、曲げた背もたれ枠）で組み、
...
- 差し替え点は `getFurnitureTexture()` の中だけ。`FurnitureDef.sprite` があればそれを返す

### 分かったこと

| | |
| --- | --- |
| 見た目 | **はっきり良くなる。** 曲線・丸み・落ち影が出る。手続き生成の階段状の脚とは別物 |
| 容量 | いす1脚（4方向）で **19KB**。実測 0.57B/px から全84家具では **約2.2MB** |
| コード | 配置・重なり順・共有 URL・セーブは**無改修**。読み込みと分岐だけ |
| ⚠️ リカラー | **使えない。** 焼いた絵には陰影が入っているので色を差し替えられない。
  いまは手続き生成に落ちるようにしたが、**色を変えた瞬間に見た目が別物に戻る**（実測で確認） |
| ⚠️ 混在 | 1脚だけ3Dにすると**明らかに浮く**。全部を焼き終えるまで出せない |
| ⚠️ 焼いた影 | 影が絵に入るので、時間帯や家具の重なりに追従しない |
| ⚠️ アバター | 髪型10×服4×色×姿勢×4方向は焼くと組み合わせが爆発する。**アバターは手続き生成のまま**が現実的 |

### 判断

- **お試しの1脚は配らない。** `?sprites=on` を付けたときだけ有効になる（既定は手続き生成）
- 進めるなら「家具84種を1つの美術方向でモデリングする」ことが本体で、これは**絵の仕事**。
  Unity に移るかどうかとは無関係に発生する
- リカラー（木地10色×張地10色）は**この路線と両立しない**。捨てるか、
  色を差し替えられる形（陰影を別レイヤーに分ける、マスク付きで焼く）に作り直すかの判断が要る
