...
| v0.3 | みせる | ❌ 未着手 | URL 共有・訪問モード・PNG 保存 |
...
**毎日来る理由（ボーナスとミッション）と集める理由（ショップ）は入りました。**
残っている穴は「見せる」だけです。

### v0.2 でやり残していること

- **計測カウンタが未実装**。ロードマップでは「v0.2 と同時」と決めていた。
  北極星指標（1日1回以上 部屋を編集したユーザー数）を測る仕組みがまだない
- **コアのユニットテストが未実装**（`core/iso`、`pathfinding`、`canPlace`）。Vitest 未導入
- 家具は52種になったが、箱の形を共有している小物は見分けがつきにくい。
  色で描き分けたが、形のバリエーション（丸い箱、脚つき、扉つき）はまだ少ない

### 次にやること

1. **v0.3 みせる** ― URL 共有と訪問モード。ここの数字がマルチプレイの投資判断になる
2. 計測カウンタ（v0.3 と同時でよいが、これ以上先延ばししない）
3. コアのユニットテスト
