...
| **v0.5** | **いきている** | ✅ **完了** | エモート10種＋表情7種、おまかせ、時間帯、家具インタラクション7種、ペット6種、スタンプ10種、**NPC の来客** |
| — | 土台 | ✅ 追加 | **PWA**（ホーム画面に追加・2回目からオフライン）。版ごとにまとめて保存し、更新は「読み込み直す」の帯で確実に入れかえる |
...
| 反応が返る | 🔶 **NPC の来客**が部屋を見てスタンプを出し、おみやげを置いていく。ただし**本物の人**の反応が返るのは v1.0（訪問中の ❤️ は押した端末のなかだけ） |
...
- ✅ **ロケット**（2×2・高さ118）を置いて押すと**月コロニー**へ行ける。**480コイン**
...
「すわる」だけが特別扱いだった箇所を `interactions` に一般化した。家具の定義には
**できることの並び**だけを持たせ、ふるまいは `data/interactions.ts` の表に集めてある。

- ✅ 7種：**すわる**／**ねる**（ベッド。足元を軸に64度倒して横になる）／
  **みる**（テレビ）／**うつる**（かがみ・ドレッサー）／**よむ**（本棚・キュリオ）／
  **かける**（ちくおんき。踊りだす）／**みずやり**（はちうえ・花びん）
- ✅ 家具を押すと**先頭のことをやる**。すわれる家具の先頭は「すわる」なので、
  今までの操作は変わらない。2つめ以降は選択バーのボタンで選ぶ
- ✅ **正面のとなりに立つ**（`core/placement.ts` の `frontTiles`）。
  裏に回ると家具でアバターが隠れてしまうため
- ✅ している間は家具の足元がゆっくり明滅する（テレビは青、音楽は金、みずやりは緑）
- ✅ 歩き出す・家具をしまう・別のことをはじめる、のどれでも自動でやめる。
  開始のたびに解除を書くと必ず漏れるので、毎フレーム1か所で見張る形にした
- ✅ **おまかせ**（放置中の自動行動）も家具を使う。置いた甲斐が見える時間になる
- ✅ 日ごとの `used` を数え、やること「家具で2回あそぶ」を追加
- ⚠️ 副産物：家具を選ぶ帯がスマホで画面からはみ出していた（`閉じる` が押せない）。
  折り返し＋狭い画面では絵柄だけにして直した

### ホーム画面に追加（PWA・実装済み）

要望を受けて入れた。この箱庭は**画像アセットがゼロ**で、通信で取るものが
HTML・JS・CSS の3つだけなので、オフライン化の費用がほとんどかからない。
1日5〜10分ちょこちょこ触る設計なので、ホーム画面から1タップで開ける価値も大きい。

- ✅ `manifest.webmanifest`（standalone・アイコン4種。アイコンは `public/icon.svg`
  から起こした PNG。Chrome の manifest 解釈でエラー0を確認）
- ✅ Service Worker はビルド時に生成（`src/pwa/swSource.ts` → `dist/sw.js`）。
  **版ごとにまとめて保存してまとめて切り替える**方式。出力名を固定しているので、
  片方だけ新しくなると「新しい HTML ＋ 古い JS」が起きうるため
- ✅ 保存領域の名前は**中身のハッシュ**。ビルド時刻だと同じ分に2回ビルドしたときに
  版が変わらない（実測で踏んだ）
- ✅ 2回目からは完全にオフラインで動く。**もらった共有 URL もオフラインで開ける**
  （部屋の中身が URL の中にあるため）
- ✅ 更新は「次に開いたとき」。遊んでいる最中に画面を作り直さない。
  それでも古いままのときのために、**配信中の版と動いている版を見比べて
  「読み込み直す」を出す道**を用意した（押すと Service Worker に取り直させる）。
  ブラウザの自動更新だけに頼ると、古い版から出られない端末が出る
- ✅ `navigator.storage.persist()` で保存を消されにくくする（部屋は端末の中だけ）
- ⚠️ **iOS ではホーム画面のアプリと Safari で保存が別**。追加すると部屋が空になる。
  共有 URL で移せるので、あそびかたと README に手順を書いた
- ⚠️ この環境では `beforeinstallprompt` が出ないため、**「ホーム画面に追加」ボタンの
  表示は未確認**（Chrome が案内を出す条件を満たしたときだけ現れる）。
  Service Worker の登録・オフライン起動・更新の帯は実測で確認済み

### ペット（実装済み）

「部屋に自分以外の生きものがいる」ことの効きは、箱庭では大きい。放置中に
動くものがもう1つ増えるので、開いた瞬間の画面が静止画にならない。

- ✅ 6種（ねこ・うさぎ・いぬ・ことり・グレーのねこ・つきのねこ）。値段は
  🪙320〜720。**いちばん安い1匹は初日には買えないが数日で届く**位置
  （data/pets.test.ts で固定）
- ✅ **家具のカタログに混ぜていない。** 置く・回す・しまう・重なり順という
  家具の約束がどれも当てはまらないため、`data/pets.ts` を別建てにした。
  買うところだけ家具と同じ形にしてある
- ✅ 絵は1つの手続きで4種を描く（`render/petArt.ts`）。違うのは耳・しっぽ・
  くちばしだけ。⚠️ 最初はしっぽが体に隠れて見えなかったので外へ出した
- ✅ **売れない。** 生きものを売り買いさせない。同じ種類は1匹まで
- ✅ ふるまいは `entities/petBrain.ts` に純粋関数として切り出した。
  乱数を引数で受けるので「はなれすぎたら必ず追いかける」「寝ながら歩かない」
  「立った状態から急に寝ない」をテストで固定できる
- ✅ 部屋を移ってもついてくる（連れているものだから）。月では低重力で浮く
- ✅ 押すとなでられる。❤️ が飛んですわる。日ごとの `patted` を数え、
  やること「ペットを2回なでる」を追加。**ペットを連れている人にだけ出す**
  （`MissionDef.needsPet`）
- ✅ セーブ v7（`pets` / `pet`）。v6 は「飼っていない」で始まる
- ✅ **共有 URL にも載る**（形式6）。訪ねた人にはその家のペットが見え、
  自分のペットは連れて行かない（おじゃましているのは相手の部屋なので）。
  **とりこんでもペットはもらえない**（もらえるとコインを払う意味が無くなる）。
  知らないペットの id は「いない」に落とす。形式1〜5 の古い URL も読める

### 供給（数字を待つあいだに足したもの）

「作る」より「測る」が先の局面なので、**土台をいじらず種類だけを足す**作業を選んだ。

- ✅ **家具 72 → 78 種**。ロードマップに「形のバリエーション（丸い箱・脚つき・扉つき）が
  少ない」と書いていた積み残しに当てた。新しい形は4つ：
  **まる**（丸テーブル2種・丸スツール。箱では出せない曲線を blob で作る）／
  **ピアノ**（白鍵の帯に黒鍵を等間隔で乗せる。猫脚つき）／
  **だんろ**（火床の中に炎。**夜は足元が光る**＝ランプと同じ扱いにした）／
  **すいそう**（水・砂・水草・魚）
- ✅ **できることを2つ追加**：**ひく**（ピアノ。v0.5 の当初リストに書いてあった項目）と
  **あたたまる**（だんろ）。すいそうは既存の「みる」を使い回した
- ✅ **スタンプ 10 → 14 種**：おはよう（太陽）／おやすみ（三日月）／さいこう（王冠）／
  おちゃどうぞ（ティーカップ）。三日月は塗りの引き算ができないので、外側と内側の
  円弧をつないだ多角形で作った
- ✅ 形の取りこぼしを見張るテストを追加（カタログの形が描き分けているものだけか）
- ✅ **壁に掛けるもの 13 → 19 種**（ここは実装以来ずっと手つかずだった）。新しい形は3つ：
  **はなづな**（放物線でたるませた花の弧）／**かざり皿**（丸。枚数はマス数から決める）／
  **かべのグリーン**（鉢から左右へ垂れる葉）
- ✅ **ペット 6 → 8 種**：**ハムスター**（小さく丸く、ほおをふくらませる）と
  **かめ**（こうら＋前に出た頭）。⚠️ かめは最初、頭をこうらの上に置いてしまい
  完全に隠れていた。頭は**前（画面では下）**へ出すのが正しい

- ✅ **きせかえ**：髪型 6 → 10 種（ポニーテール・みつあみ・ひめカット・くるくる）、
  ふくのかたち 2 → 4 種（パーカー・セーラー）。ここも v0.1 以来ずっと手つかずだった。
  **自己表現は3本の柱の1本目**なので、供給としては効きが大きい
  - セーラーはスカートだけ「ズボン／くつした」の色を使い、上下で色が分かれて見えるようにした
  - ⚠️ ふくのかたちは共有 URL に**番号**で載る。あとから足せるよう `OUTFITS` の並びを
    固定し（0=シャツ / 1=ワンピース は形式1のころと同じ）、知らない番号はシャツに落とす
  - ⚠️ 「かみがたの番号を範囲に収める」テストが `<= 5` と数字を直書きしていて、
    髪型を足したら落ちた。**いくつあるか**から取るように直した（床・壁の柄で以前やった直しと同じ）

- ✅ **ゆか 6 → 10 種・かべ 6 → 10 種**。ここは色を替えるだけでは「別の床」に見えないので、
  **柄の型**（`pattern`）を持たせて1マスの中の描き方から変えた
  - ゆか：いたばり（列ごとに色を替えて継ぎ目を入れる）／ちいさいタイル（4分割）／
    よせぎ（中央に菱形）／だいりせき（目地つき）
  - かべ：ストライプ／こしかべ（腰の高さに見切り＋鏡板）／みずたま／レンガ（互い違いの段）
  - 壁面の柄は「沿った割合 t と高さ h」で置く。平行四辺形なので両端を t で混ぜればよい
    （`core/wall.ts` と同じ考え方）
  - テーマも3つ追加（カフェ・ドット・アトリエ）
  - ⚠️ 番号は共有 URL とセーブに載っているので**末尾にだけ足す**。
    先頭6つの並びが変わっていないこと・月の番号がその柄を指したままであること・
    テーマの番号が実在することをテストで固定した

### 次にやること

**v0.5 まで作り切りました。ここから先は「作る」より「測る」が先です。**
残っている山（v1.0 のマルチプレイ、L段の共有月面広場）はどちらもサーバーが要り、
運用コストと責任が発生します。**着手条件は共有の数字**なので、先に測ります。

1. **判断ゲートの計測**：月の部屋の設置家具数の中位値が1週間で8個に届くか。
   届かないなら行き先を増やすのは止めて、1部屋の密度に投資を戻す
2. **v0.3 の数字**（共有 URL が配られて開かれているか）。伸びていれば v1.0 へ、
   伸びていなければ「見せる」の入口をもう一段作り直す
3. 数字を待つあいだに手を動かすなら、**供給を止めない側**（家具・スタンプ・
   ペットの種類を足す、シーズン家具の仕組み）。土台をいじる作業は入れない
...
- ❌ **ロケットを高くしすぎる。** これは一度やってしまった（1500コイン＝実測で10日以上）。
  自分で「関所にしない」と書いておきながら関所を作っていた。
  **値段は「初日の所持（420＋ボーナス45）では足りないが、やること1件で届く」位置に置く**
  （いま480）。この意図は `data/furniture.test.ts` でテストに固定してある
...
- ⚠️ **家具の id を衝突させた。** かべどけいを `clock` で追加したが、既にあった
  「おきどけい」（床の飾り）と同じ id だった。`getDef('clock')` は後から定義した壁時計を
  返すので、**おきどけいが実質使えない状態**になっていた。
  カタログの id 重複を検査するテストを書いたら即座に見つかった。
  壁側を `wall-clock` に改名し、**すでに壁に掛かっている `clock` だけを読み替える**表
  （`resolveWallId`）を足して、既存プレイヤーの壁時計を落とさずに直した。
  **カタログにデータ行を足すときは id 重複のテストを通すこと**
- ⚠️ ロケットがショップ72件の66番目に埋もれていて、気づかれない。
  買える額になった時点で1回だけ知らせるようにした（`maybeHintRocket`）。
  並び順をいじると値段順が壊れるので、案内で解いている
...
- ✅ **家具インタラクション（実装済み）**：7種（すわる／ねる／みる／うつる／よむ／かける／
  みずやり）。`interactions` に一般化し、ふるまいは `data/interactions.ts` の表に集めた。
  そばに立つものは家具の**正面**に立つ（裏に回ると隠れて何をしているか見えないため）
- ✅ **時間帯（実装済み）**：端末の時計で 朝(5-10) / 昼(10-17) / 夕(17-19) / 夜(19-5)。
  床と壁に色調をかけ、**夜はランプの足元に灯りが落ちる**。上部バーに 🌅☀️🌇🌙 を出す。
  **月コロニーは色調をかけない**（いつも星空なので、朝夕でドームの外が明るくならないように）
- ✅ **ペット（実装済み）**：6種。うろうろ・すわる・ねる・追いかけるを自分でやり、部屋を移っても
  ついてくる。押すとなでられる。ふるまいは `entities/petBrain.ts` の純粋関数に切り出してある。
  **共有 URL にも載る**（形式6。とりこんでもペットはもらえない）
- ✅ **NPC の来客（実装済み）**：遊びはじめて1〜2分後にひとり来て、家具のそばを
  見てまわり、スタンプでほめて、1分ほどで帰る。その日の1人目は**おみやげ**を置いていく。
  自分の家にだけ来る（月コロニーと、人の部屋を訪ねている間は来ない）。
  段取りは `entities/guestPlan.ts` の純粋関数で、**必ず帰る**ことをテストで固定してある
- 放置アニメ（あくび、伸び、鼻歌）── おまかせの行動リストに足せる
- ✅ **スタンプ（実装済み）**：10種（だいすき／すごい／たのしい／すてき／わあ！／
  なあに？／あせあせ／ありがとう／おめでとう／ねむい）。吹き出しに絵柄を1枚出す。
  絵文字ではなく自分で描いているので、端末によって見た目が変わらない。
  **人の部屋でも出せる**（言葉を打たずに反応を返せる入口）
...
- ✅ `FurnitureDef.seatHeight` を `interactions: InteractionKind[]` に一般化した。
  座れる家具は書かなければ「すわるだけ」とみなすので、既存の16脚は1文字も触っていない
- ✅ 時間帯は読み通り `RoomView.redraw()` に色調フィルタを1段足すだけで済んだ（`core/timeOfDay.ts`）。
  1分ごとに見張るので、遊んでいる最中に夕方から夜へ変わる。
  ⚠️ 起動時は「変化があったときだけ反映」だと**月コロニーで表示が消えなかった**（最初から null のため）。
  初回だけ強制で反映する引数を足して塞いだ
- ✅ ペットは読み通り `Avatar` の簡易版で足りた。経路探索はそのまま流用し、絵だけ
  `render/petArt.ts` に新しく起こした（4種を1手続きで描く）
...
「あそびに来たプレイヤーの部屋へ飛べるか」への答え。**本物のプレイヤーはサーバーが要る**（v1.0）
ので、その手前を NPC で埋めた。訪問のしくみ（読み取り専用・訪問バー・じぶんの部屋へ）は
共有 URL のものをそのまま使い、入り口だけ `#g=<id>` を足してある。

### 決めたこと

- **顔ぶれは6人で固定**。毎回ランダムな見た目だと「また来た」が起きず、誰にもならない。
  固定すると「しおりちゃん、また来たね」になり、**その人の部屋へ行ってみたい**まで つながる
- **部屋は手で組む**。自動生成した部屋は見ればすぐ適当だと分かり、1回で動機が尽きる。
  数は少なくていいので置き方に意図のある部屋にした（`data/friends.test.ts` で置けることを固定）
- **ものが少ない部屋を1つ入れる**（つむぎ）。全部が飾り込まれていると
  「埋めないといけない」空気が出る。これはテストで固定してある
- **部屋には主が居る**。主のいない部屋を見せても「会いに行った」感じにならない。
  おきゃくさんの仕組み（Guest）に `stay` を足して、帰らないようにした
- ⚠️ **とりこめない**。とりこむと置いてある家具が持ちものに増える（引かれない）ので、
  ゲーム内のボタンで何度でも行ける先でこれを許すと **しまう→うる でコインが無限に湧く**。
  `canImportRoom()` に切り出してテストで固定した

### やらなかったこと

- **部屋の自動生成**。上のとおり、見ればすぐ分かるので動機にならない
- **NPCの部屋を共有 URL に載せること**。載せると「人からもらった共有 URL」と
  見分けが付かなくなり、とりこみを止められない。`#g=<id>` は id しか載せていない

---

## 付録：3Dモデル → 等角スプライト化（家具ぜんぶ完了）

「Unity に移してリアルにできるか」への答えとして、**Unity に移らずに絵の質を上げる道**を採った。
床に置く家具 66種と壁に掛けるもの19種を、すべて 3Dモデルから焼いた絵に置き換えた。

### やりかた

- `tools/sprite-render/` がヘッドレス Chromium の中で Three.js を動かし、
  **ゲームとまったく同じ射影**で4方向ぶんレンダリングして PNG にする
- 射影は方位45度・仰角30度の正射影。`1ワールド単位 = 45.25px（横）/ 39.19px（高さ）`にすると
  `画面x = (gx-gy)*32 / 画面y = (gx+gy)*16 - z` と一致する。アンカー（マスの奥角）の
  ピクセル位置も `furnitureTexture.ts` と同じ式で合わせたので、**配置・重なり順のコードは1行も変えていない**
- 形は `shape` ごとの組み立て関数（14種）で、大きさ・高さ・座面高から作る。
  手続き生成の `paint()` と同じ構成にしてあるので、「同じ家具の、丸みと陰影があるほう」になる

### リカラーを殺さない焼き方（試作のときの宿題）

試作では「焼いた絵には陰影が入っているので色を差し替えられない」のが最大の障害だった。
**陰影とマスクを別の層にして1枚に詰める**ことで解決した。

- 左（陰影）＝本体色・張地色を白にして焼いた絵 / 右（マスク）＝ R:本体色 G:張地色 A=0:色を変えない
- ゲーム側で `out = 陰影 × (白×(1-R-G) + 本体色×R + 張地色×G)`
- 焼いた絵に色が入らないので、**形が同じなら色ちがいは1枚を共有できる**。66種 → 53枚

白い面が 1.0 で潰れると色を掛けても白いままになるので、焼くときに明るさを測っている
（`焼き飽和 0.07% / 明るさ p95 の中央値 224`）。

### 測った数字

| | |
| --- | --- |
| 見た目 | 曲線・丸み・落ち影が出る。猫脚・背もたれ枠・ロケットの円錐は手続き生成では出せない |
| 容量 | 228枚 / **1.43MB**（形の共有と、色を焼かないぶんで試作時の見積 2.2MB より小さい） |
| コード | 配置・重なり順・共有 URL・セーブは**無改修**。読み込みと合成だけ |
| リカラー | **そのまま動く**（上記） |
| 起動 | preload に入れず、部屋を出してから裏で読む。読めるまでは手続き生成のまま |
| オフライン | まとめ取り（PRECACHE）には入れず、使われたものだけ版ごとの保存領域へ足す |
| ⚠️ 焼いた影 | 影が絵に入るので、時間帯や家具の重なりには追従しない |
| ⚠️ アバター | 髪型10×服4×色×姿勢×4方向は組み合わせが爆発する。**アバターは手続き生成のまま** |

### 途中で見つけて直したこと

- **角丸の箱の高さが 1.5倍になっていた**。`ExtrudeGeometry` は「押し出し量 + 面取りぶん」だけ
  大きくなるのに、そのぶんを引かずに作っていた。全家具が浮いて見えていた原因。
  焼くときに**枠からのはみ出しを数えて報告する**ようにして、同じ間違いに気づけるようにした
- `metalness` の高い金は環境マップが無いと暗い緑になる。部屋を模した上下グラデーションを
  1枚作って IBL に使い、金彩が読めるようにした

### 壁に掛けるもの

壁の上の座標 `(u, h)` は `ワールドx = u/32 / ワールドy = h/39.19 / ワールドz = 壁からの出っぱり`
と置くと、床とまったく同じカメラで焼ける。**左の壁は右の壁の左右反転**（`core/wall.ts` の
写し方がちょうど鏡になる）なので、焼くのは右のぶんだけで、ゲーム側が合成のときに反転する。

枠ものは「1枚の箱＋中身」ではなく**4本の棒**で組む必要があった（1枚の箱にすると中身が隠れて、
絵もまどもただの金色の板になる。実際やらかした）。

### アバターとペット

丸ごと焼くのは無理だった。歩きの振り・呼吸・まばたき・16種のモーションが**連続した値**で
動くので、姿勢ごとに焼くと組み合わせが爆発するうえ、モーションの途中の絵が作れない。

そこで **形は手続きのまま、面の陰影だけを3Dから焼いた部品で与える**ことにした。
部品（球・棒・板・筒の4つ）は真正面からの正射影で焼いてあるので、**シルエットは
平らな絵とまったく同じ**まま、丸みだけが乗る。色は Phaser の tint（掛け算）で乗るので
マスクも合成も要らない。

- 差し替えの単位は `render/parts.ts` の `Painter` 型。Graphics もそのまま通るので、
  部品が読めていないとき・`?sprites=off` のときは今までどおり平らに描かれる
- 描き直しの費用は 1回 0.17ms（平らなときは 0.09ms）。毎フレーム描き直しても1%に届かない
- きせかえのプレビューとペットのショップの絵も、同じ部品で描く（`CanvasPartPainter`）。
  ここだけ平らだと、選んだ姿と部屋に立っている姿が別物になってしまう
- 部品は4枚で 30KB。姿勢ごとに焼いていたら 100枚では足りなかった

小さい形（耳・髪の毛先・スカートのひだ）は多角形のままにしてある。
3px の三角に陰影を入れても濁るだけで、良くならない。

### 残り（3D化しないもの）

- **ゆか・かべの柄**。部屋の面そのものなので、家具とは別の話
