...
| **v0.5** | **いきている** | 🔶 **5/7** | ✅ エモート10種＋表情7種、✅ おまかせ（放置中の自動行動）、✅ 時間帯（朝昼夕夜・夜はランプが点く）、✅ **家具インタラクション7種**、✅ **ペット6種**／未：エモートのスタンプ化、NPC の来客 |
...
1. **エモートのスタンプ化**（吹き出しに絵柄を出す）。これで v0.5 の「入れるもの」が
   NPC の来客を除いて片づく
2. **判断ゲートの計測**：月の部屋の設置家具数の中位値が1週間で8個に届くか。
