...
ロードマップを書いてから7コミット。**そのすべてが v0.5（演出）と見た目の作業**で、
勝ち筋として置いた v0.2 / v0.3 には1行も入っていません。

| Ver | テーマ | 状態 | 中身 |
| --- | --- | --- | --- |
| v0.1 | 飾る（土台） | ✅ 完了＋磨き込み | 等角の部屋、家具21種のロココ調化、配置/移動/回転/しまう、きせかえ、保存、Pages 公開、ピンチズーム |
| **v0.2** | **あつめる** | ❌ **未着手** | コイン・ショップ・日替わりミッション・家具50種・売却 |
| **v0.3** | **みせる** | ❌ **未着手** | URL 共有・訪問モード・PNG 保存 |
| v0.4 | そだてる | ❌ 未着手 | 部屋拡張・壁面レイヤー・リカラー |
| v0.5 | いきている | 🔶 2/6 実装済み | ✅ エモート10種＋表情7種、✅ おまかせ（放置中の自動行動）／未：家具インタラクション、時間帯、ペット、スタンプ |
| v1.0 | いっしょに | ❌ 未着手 | マルチプレイ（v0.3 の数字が条件） |

### これが意味すること

- **アバターは十分かわいくなり、放置しても動くようになった**（柱1「自己表現」と滞在時間に効いた）
- しかし**翌日また来る理由はゼロのまま**。北極星指標「1日1回以上 部屋を編集したユーザー数」を
  動かす機能（獲得・目的・共有）が1つも入っていない
- ループの穴は下の表のまま塞がっていない

### 次にやること（順番を戻す）

1. **セーブのマイグレーション基盤** ― v0.2 の前提。これ以上機能を足す前に片づける
2. **v0.2 あつめる** ― コイン、ショップ、日替わりミッション、家具50種
3. **v0.3 みせる** ― URL 共有。ここの数字でマルチプレイの投資判断をする

v0.5 の残り（家具インタラクション、時間帯、ペット）は**v0.2 の後でよい**。
演出は滞在時間を伸ばすが、再訪を作るのは獲得と共有です。
...
ループに関して実装済みなのは**「飾る」**だけです。エディタとしては成立していて、
アバターの演出（エモート・おまかせ）も入りましたが、**ゲームのループはまだ閉じていません**。
