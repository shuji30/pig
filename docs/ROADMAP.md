...
| v0.4 | そだてる | ❌ 未着手 | 部屋拡張・壁面レイヤー・リカラー |
| v0.4.5 | とびだす | ❌ 未着手（計画に追加） | ロケット家具 → 月コロニー（2つめの部屋） |
...
- ✅ **コアのユニットテスト**（Vitest 53件）：`core/iso`、`core/pathfinding`、`core/placement`、`state/share`。
...
1. **v0.4 そだてる** ― 部屋サイズの可変化と `SaveData` の複数部屋化を最初にやる（v0.4.5 の前提）
2. 壁面レイヤーとリカラー（表現の天井を上げる。供給効率が最も高い）
3. その後 **v0.4.5 とびだす**（ロケット／月コロニー）
...
| **M 本命** | ＋1週 | 低重力歩行（ふわっと跳ねる）、地球が見える窓、常時星空、月面限定ミッション、月面限定家具の入手経路 |
...
- ⚠️ **`SaveData` の複数部屋化が最初のタスク。** 現在 `floor` / `wall` / `items` はセーブ直下にあります。
  `rooms: Record<string, RoomData>` ＋ `currentRoom: string` へ持ち上げ、`migrate(v2 → v3)` で
  既存の内容を `rooms.home` に移す。**ここを後回しにすると全機能を触り直すことになります**
- 部屋サイズの可変化（7章）と**同じタイミングで**やる。月は 10×10 など地上と違う形にできると差が出る
- ロケットは `interactions` の一種（`kind: 'travel'`）として実装する。座る仕組みの拡張で乗り込み姿勢が作れる
- 発進演出は `RoomScene` のフェード＋カメラ移動で足りる。**専用シーンは作らない**
- 低重力歩行は `Avatar` の歩行に上下オフセットを1本足すだけ。`AutoPlay` は無改造で動く
- 星空は `RoomView.redraw()` の色調フィルタ（v0.5 の時間帯と同じ場所）に月用プリセットとして入れる
- 月家具は手続き生成なので **`data/furniture.ts` にデータ行＋色指定を足すだけ**。ここは本作の構造的な強み
...
| 項目 | いつやるか | 理由 |
| --- | --- | --- |
| ~~セーブのマイグレーション基盤~~ | ✅ v0.2 で完了（v3 まで運用中） | これ以降すべての機能追加が `SaveData` を触る |
| ~~計測カウンタ~~ | ✅ v0.3 で完了 | 後付けは必ず後回しになる（実際に1バージョン遅れた） |
| ~~コアのユニットテスト（`core/iso`、`pathfinding`、`canPlace`）~~ | ✅ v0.3 で完了 | 座標と占有マスの回帰は目視で気づけない。Vitest 53件。`canPlace` は `core/placement.ts` に切り出した |
| 家具どうしの重なり順のトポロジカルソート | **v0.4 の前** | 大型家具・壁面家具を増やすと近似が破綻する |
| 部屋サイズの可変化 | v0.4 の最初 | 定数のまま機能を積むと後で全部触ることになる |
| `SaveData` の複数部屋化（`rooms` / `currentRoom`） | **v0.4 の最初**（部屋サイズ可変と同時） | 月コロニー・複数部屋・共有の全部がここに乗る。後からの分割は全機能に波及する |
