...
| **v0.5** | **いきている** | 🔶 **6/7** | ✅ エモート10種＋表情7種、✅ おまかせ（放置中の自動行動）、✅ 時間帯（朝昼夕夜・夜はランプが点く）、✅ 家具インタラクション7種、✅ ペット6種、✅ **スタンプ10種**／未：NPC の来客 |
...
1. **判断ゲートの計測**：月の部屋の設置家具数の中位値が1週間で8個に届くか。
   ...
   **v0.5 は NPC の来客を残すだけなので、ここから先は「作る」より「測る」が先**
2. NPC の来客（v0.5 の残り）。おまかせの行動リストと同じ仕組みに乗せられる
